...
async function mutate(key, fn) {
  if (!useKV) {
    ...
    let release;
    locks[key] = new Promise(resolve => { release = resolve; });
    try {
      const value = await get(key);
      const result = await fn(value);
      if (result !== undefined) await set(key, result);
      return result;
    } finally {
      delete locks[key];
      release();
  ...
  // KV mode: mutex hanya生效 dalam satu instance, lebih baik dari nothing
  const value = await get(key);
  const result = await fn(value);
  if (result !== undefined) await set(key, result);
  return result;
...
module.exports = { get, set, del, initLocal, mutate, HttpError };
