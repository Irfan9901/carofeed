...
- **Intruksi Tambahan** (`#inp-customstyle`): placeholder "contoh: hand-drawn texture, maskot lucu" — content split by comma, ditambahkan ke `activeStyleTags()` (dedupe case-insensitive terhadap tag preset), keluar di JSON output (`style_tags`) serta modul "Style Direction" pada copy prompt. Juga diinjeksi sebagai `Instruksi tambahan: ...` dalam AI user prompt. Copy prompt hanya memiliki SATU blok style ("The ENTIRE composition MUST follow the style: ...") — tanpa blok `preset.label` yang menduplikasi; tag preset + tag custom + hex palette digabung dalam satu perintah.
- **Cover slide hook** (`#inp-cover-hook`): placeholder "cth: 5 Menit Sehari untuk Kesehatan Mentalmu" — field for exact cover headline. Auto-filled from AI `coverHook` field when "Generate ide" clicked. Injected into AI user prompt as `Cover slide headline HARUS persis: "..."` (mode carousel) atau `Poster headline HARUS persis: "..."` + perintah tuntas tanpa kelanjutan (mode poster / jumlah slide = 1). After AI generates slides, `slides[0].headline` is overridden with this value. User can edit/customize manually.
- **Poster Tunggal (1 slide):** saat `state.slideCount === 1`, `aiGenerateSlideContent()` memakai `system_poster`/`user_poster` (default di `DEFAULT_PROMPTS`, editable admin di modal "Kelola Prompt AI" → Prompt Tambahan). Seluruh parameter brief (topic, purpose, audience, slideCount, brandNote, niche, subniche) + customStyle + coverHook tetap disertakan — sama seperti mode carousel. Hasil: array 1 elemen yang tuntas tanpa kelanjutan.
...
- Used in generate buttons ("Menggali ide…", "Membuat cover hook…", "AI menyusun slide…", "AI menyusun poster…")
